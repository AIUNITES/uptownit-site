...
const CloudDB = {
  ...
  init(options = {}) { Object.assign(this.config, options); const savedApiUrl = localStorage.getItem(this.config.storagePrefix + 'apiUrl'); if (savedApiUrl) this.config.apiUrl = savedApiUrl; console.log(`☁️ CloudDB initialized for ${this.config.siteName}`); return this; },
  ...
  async submit(type, data, email = '') {
    ...
    try {
      ...
      formData.append(this.config.entryIds.message, this.packData(type, data));
  ...
  async fetch(type = 'users') {
    ...
    if (!this.isEnabled()) return { success: false, reason: 'disabled' };
    try { return { success: true, data: await (await fetch(type ? `${apiUrl}?type=${type}` : apiUrl)).json() }; }
    catch (err) { return { success: false, error: err.message }; }
  },
  ...
  updateStatusBadge() { const badge = document.getElementById('clouddb-status'); if (!badge) return; const enabled = this.isEnabled(); badge.style.background = enabled ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)'; badge.style.color = enabled ? '#22c55e' : '#ef4444'; badge.textContent = enabled ? '● Online' : '○ Offline'; },
  renderStatusBadge(containerId) { const container = document.getElementById(containerId); if (!container) return; const enabled = this.isEnabled(); container.innerHTML = `<div style="display:inline-flex;align-items:center;gap:8px;padding:6px 12px;background:${enabled?'rgba(34,197,94,0.1)':'rgba(255,255,255,0.05)'};border-radius:20px;font-size:0.8rem;"><span style="width:8px;height:8px;border-radius:50%;background:${enabled?'#22c55e':'#666'};"></span><span style="color:${enabled?'#22c55e':'#888'};">${enabled?'Cloud Sync':'Offline Mode'}</span></div>`; }
};
